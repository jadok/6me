const getTotalFromQtyUnitPrice = (qty: number, unitPrice: number, nbr: number): number => qty * unitPrice * nbr;
const getQtyFromTotalUnitPrice = (total: number, unitPrice: number, nbr: number): number => nbr * unitPrice / total;
const getUnitPriceFromTotalQty = (total: number, qty: number, nbr: number): number => total / qty / nbr;
