...
const analyzeDataByAttribute = (attribute: string, data: Array<Buy>) =>
  data.reduce((dataFiltered: Record<string, number>, cur) => cur.products.reduce((accP: Record<string, number>, curP: Product) => {
      ...
      return accP;
    }, dataFiltered), {} as Record<string, number>);


const Analyze = () => {
  const data = useProductQuery().data!

  const dataByCategories = useMemo(() => dataToPieData(analyzeDataByAttribute('category', data.products as Array<Buy>)), [data]);
  const dataByOrigins = useMemo(() => dataToPieData(analyzeDataByAttribute('origine', data.products as Array<Buy>)), [data]);
  return (
    <div>
      <fieldset>
