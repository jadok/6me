...
const cleanFilename = (file: string): string =>
  getFileWithoutExtension(file).replace(postsDirectory + sep, '');

type Items = {
  [key: string]: string
}

export const getPostSlugs = async (): Promise<Array<string>> =>
  new Promise((resolve, reject) =>
    filewalker(postsDirectory, (err, files) => {
      if (err) reject(err);
      resolve(files);
    })
  );

export const getPostBySlug = (slug: string, fields: string[] = []): Promise<Items> => {
...
export const getAllPostsPaths = async (
  _fields: string[] = []
): Promise<Array<Partial<ContentType>>> => {
  return getPostSlugs().then((files) =>
    files.map((file) => {
