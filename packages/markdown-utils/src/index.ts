...
export const endHeading = (data: Chapter[], heading: Heading, linesContent: string, currentHeadingNbr = 1) => {
  const headingNbr = Number.isInteger(heading) ? heading : (heading as string[])[0].length;
  const currentLength = data.length;
  if (currentHeadingNbr < headingNbr) {
    if (currentLength) {
      endHeading(data[currentLength - 1].subs, heading, linesContent, currentHeadingNbr + 1);
    }
